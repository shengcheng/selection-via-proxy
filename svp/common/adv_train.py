...
def run_training(model: nn.Module, optimizer: Optimizer, criterion: nn.Module,
                 ...
                 num_step: int, step_size: float, epsilon: float, beta: float,
                 ...
                 run_dir: Optional[str] = None,
                 checkpoint: str = 'last'
                 ) -> Tuple[nn.Module, AccuracySplits, AccuracySplits, TimeSplits]:
    """
    ...
    best_robustness = -1
    best_train_robustness = -1
    best_dev_robustness = -1
    best_test_robustness = -1
    ...
    for nepochs, learning_rate in zip(epochs, learning_rates):
        ...
        for epoch in range(start_epoch, end_epoch):
            ...
            global_step, train_accs, train_robustness, epoch_train_time = run_epoch(
                epoch, global_step, model, train_loader, device,
                num_step, step_size, epsilon, beta,
            ...
            if train_robustness[0].avg > best_train_robustness:
                best_train_robustness = train_robustness[0].avg

            if dev_loader is not None:
                # Run validation
                _, dev_accs, dev_rob, epoch_dev_time = run_epoch(
                ...
                if dev_rob[0].avg > best_dev_robustness:
                    best_dev_robustness = dev_rob[0].avg

            if test_loader is not None:
                # Run testing
                _, test_accs, test_rob, epoch_test_time = run_epoch(
                ...
                if test_rob[0].avg > best_test_robustness:
                    best_test_robustness = test_rob[0].avg

            # Check if performance has improved for early stopping.
            current_accuracy = train_accs[0].avg
            current_robustness = train_robustness[0].avg
            accuracy_mode = 'train'
            if dev_loader is not None:
                current_accuracy = dev_accs[0].avg
                current_robustness = dev_rob[0].avg
                accuracy_mode = 'dev'

            is_best = current_robustness > best_robustness
            if is_best:
    ...
    accuracy_splits = AccuracySplits(best_train_accuracy,
                                     best_dev_accuracy,
                                     best_test_accuracy)
    robust_splits = AccuracySplits(best_train_robustness,
                                   best_dev_robustness,
                                   best_test_robustness)
    time_splits = TimeSplits(train_time, dev_time, test_time)
    return model, accuracy_splits, robust_splits, time_splits
...
def run_epoch(epoch: int, global_step: int, model: nn.Module,
              ...
              batch_callback: Optional[Callable] = None,
              fp16: bool = False
              ) -> Tuple[int, List[utils.AverageMeter], List[utils.AverageMeter], timedelta]:
    """
    ...
    robustness = [utils.AverageMeter() for _ in top]
        ...
        with torch.set_grad_enabled(train):
            ...
            for batch_index, (indices, inputs, targets) in enumerate(wrapped_loader):  # noqa: E501
                ...
                if train:
                    ...
                    loss, loss_natural, loss_robust, adv_inputs = trades_loss(model,
                                                                  x_natural=inputs,
                                                                  y=targets,
                ...
                for i, count in enumerate(top_correct):
                    accuracies[i].update(count.item() * (100. / batch_size), batch_size)
                for i, count in enumerate(adv_top_correct):
                    robustness[i].update(count.item() * (100. / batch_size), batch_size)# noqa: E501
                ...
                if output_file is not None:
                    ...
                    for i, k in enumerate(top):
                        ...
                        result[f'adv_top{k}_correct'] = adv_top_correct[i].item()
                        result[f'adv_top{k}_accuracy'] = robustness[i].val
                    if train:
    ...
    return global_step, accuracies, robustness, total_time
