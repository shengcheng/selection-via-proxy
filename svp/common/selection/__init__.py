...
from svp.common.selection.k_center_greedy_adv import k_center_greedy_avg_adv, k_center_greedy_minmax_adv
...
def adv_select(model: nn.Module, dataset: Dataset, current: np.array,
           pool: np.array, budget: int, method: str, batch_size: int,
           device: torch.device, device_ids: Tuple[int, ...], num_workers: int,
           use_cuda: bool,
    ...
    if method == 'random':
        ...
        new = k_center_greedy_minmax_adv(features, np.arange(len(current)), budget)
        assert (new >= len(current)).all()
        new = subset_indices[new]
    elif method == 'forgetting_events':
        assert nevents is not None
...
def _calc_preds_and_features_adv(
        model: nn.Module, dataset: Dataset, subset: np.array, batch_size: int,
        num_workers: int, device: torch.device, device_ids: Tuple[int, ...],
        use_cuda: bool,
        num_step: int, step_size: float, epsilon: float, beta: float,
